...
import LinearAlgebra: lu, lu!, ldiv!
using SparseArrays
import SparseArrays: getcolptr

include("BigRationals.jl")
using .BigRationals
...
function rational_lu!(B::SparseMatrixCSC{BigRational}, col_offset, check::Bool=true)
    ...
    @inbounds begin
        for k in 1:minmn
            ...
            if iszero(piv)
                ...
                return LU{Rational{BigInt},SparseMatrixCSC{Rational{BigInt},Int}}(B, collect(1:minmn), convert(BlasInt, k-1))
            end
    ...
    return LU{Rational{BigInt},SparseMatrixCSC{Rational{BigInt},Int}}(Rational{BigInt}.(B), Vector{BlasInt}(1:minmn), convert(BlasInt, info))
end

# function lu(A::SparseMatrixCSC{<:Rational}, pivot::Union{Val{false}, Val{true}} = Val(false); check::Bool = true)
function rational_lu(A::SparseMatrixCSC, check::Bool = true)
    ...
    # end
    isempty(J) && return LU{Rational{BigInt},SparseMatrixCSC{Rational{BigInt},Int}}(A, Int[], convert(BlasInt, 0))
    m, n = size(A)
    minmn = min(m, n)
    if J[1] != 1 || I[1] != 1
        check && checknonsingular(1, Val(false)) # TODO update with Pivot
        # return LU{eltype(A), typeof(A)}(A, collect(1:minmn), convert(BlasInt, 1))
        return LU{Rational{BigInt},SparseMatrixCSC{Rational{BigInt},Int}}(A, collect(1:minmn), convert(BlasInt, 1))
    end
    ...
    B = sparse(I, J, BigRational.(V)) # TODO update with Pivot
